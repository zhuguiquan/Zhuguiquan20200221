...
public class MlssAdapter extends RecyclerView.Adapter<MlssAdapter.RxxpViewHolder> {
    ...
    private List<ShouyeBean.ResultBean.MlssBean.CommodityListBeanXX> mRxxpscommodityList;

    public MlssAdapter(List<ShouyeBean.ResultBean.MlssBean.CommodityListBeanXX> mlsssCommodityList) {

        mRxxpscommodityList = mlsssCommodityList;
    }

    @NonNull
    @Override
    public RxxpViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View inflate = View.inflate(parent.getContext(), R.layout.mlss_item, null);
        return new RxxpViewHolder(inflate);
    }

    @Override
    public void onBindViewHolder(@NonNull RxxpViewHolder holder, int position) {
        ShouyeBean.ResultBean.MlssBean.CommodityListBeanXX commodityListBean = mRxxpscommodityList.get(position);
